...
新增 `agent/agentSettingsStore.ts`、`agent/types.ts`、`AgentSettings.tsx` 和 `agentApi.ts`。公共数据类型放在 `packages/shared`。
...
- 设置接口不返回密钥明文。
- OpenCode command 与 SDK 使用确定版本，由 `pnpm install` 安装，只监听 `127.0.0.1`。
- 安装检查不调用模型。

验收测试：设置在重启后保留；缺少 API Key 时不能启动 run；响应、错误、日志和 trace 不含密钥。

## 6. 完成 Agent 运行与 trace

状态：待开发。

新增 `agent/agentRuntime.ts`、`agent/openCodeRuntime.ts`、`agent/agentRunManager.ts`、`agent/traceStore.ts` 和 `AgentPanel.tsx`。run、session 与 trace 的公共数据类型放在 `packages/shared`。

实现要求：

- `AgentRuntime` 定义创建 session、继续 session、启动 run、订阅事件、取消、查询 diff 和释放资源，不包含 OpenCode 专用字段。
- `OpenCodeRuntime` 实现 `AgentRuntime`，管理 OpenCode 进程、SDK client、session、SSE、diff 和取消。
