...
function presentTrace(events: AgentTraceEvent[]) {
  ...
  switch (event.type) {
    ...
    case "file_changes": {
      const files = Array.isArray(event.data?.files)
        ? event.data.files.map((file) => recordValue(file)?.file).filter((file): file is string => typeof file === "string")
        : [];
      return entry(event, `${files.length || "Project"} ${files.length === 1 ? "file" : "files"} changed`, files.join(", "), "success");
    }
...
function entry(event: AgentTraceEvent, title: string, detail?: string, tone: TraceEntry["tone"] = "neutral"): TraceEntry {
