...
- 项目首页：打开已登记项目、创建空白项目、导入服务端已有目录、导入 ZIP。
...
- 代码编辑：使用 Monaco Editor 和 Yjs 同步多人文本编辑、光标和选区。
- 实时协作：显示成员、当前文件、聊天消息和 Activity。
...
1. 在 SimpleRCPv2 仓库根目录运行 `pnpm dev`。
2. 打开 `http://127.0.0.1:5173`。
3. 从项目列表打开 Demo、创建空白项目、导入 ZIP，或者填写服务端已有目录的绝对路径。
4. 填写显示名称和可选角色，进入项目工作区。
5. 与其他成员共同编辑文件、聊天和使用终端。
