...
- 左侧 IDE 风格文件树，支持折叠、展开、新建、重命名和删除文件或文件夹。
- 中间使用 Monaco Editor 编辑代码。
- 多个浏览器用户可以实时看到代码内容变化。
...
1. 在左侧展开文件夹并点击文件，中间会打开编辑器标签页。
2. 两位用户打开同一个文件后，一方修改代码，另一方会看到实时内容变化。
3. 移动光标或选择代码，另一方会在 Monaco 中看到带姓名的远端光标和选区。
4. 在右侧 Chat 页面发送消息，所有协作者会看到同一份聊天记录。
5. 在 Team 页面查看成员状态和 Activity，例如谁加入、离开、打开或编辑了文件、发送了什么消息、运行了什么命令。
6. 在底部 Terminal 运行项目测试或构建命令。
...
- 当前文件同步发送完整文件内容，还没有使用 CRDT 或 OT 处理复杂并发冲突。
